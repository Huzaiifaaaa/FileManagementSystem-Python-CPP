...
def getHelp():
    print(
        "ls [directory] - Display a list of files and subdirectories in a directory.")
    ...
    print(
        "write [filename] [m] [p] [data]- Writes to the file specified in [filename].")
    print("read [filename] [m] [p] - Reads the file specified in [filename].")
    print(
        "truncate [filename] [p]- Truncates the file specified in [filename].")
        ...
        if command == "mkdir":
            ...
            if (number >= 2):
                ...
                if exists:
                    ...
                    # print("children of created node: ", node.children)

                    # print("children of appended node: ",
                    #       current.children[-1].children)

        elif command == "ls":
            if (len(current.children) != 0):
                print("No.\tName\tisFile\tSize\t\tCreated At\t\t\tModified At")
                for i in range(len(current.children)):
                    print(str(i+1)+"\t"+current.children[i].name+"\t"+str(current.children[i].isFile)+"\t"+str(sys.getsizeof(
                        current.children[i])) + "\t"+current.children[i].createdAt+"\t"+current.children[i].modifiedAt)
                print("\n")
            ...
            if (number >= 3):
                ...
                if exists:
                    if current.children[index].isOpen == False:
                        ...
                        current.children[index].data = current.children[index].data[:int(
                            mode)]
                        current.modifiedAt = str(datetime.now())
                else:
                    print("File does not exist.")
